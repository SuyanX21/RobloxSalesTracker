...
function initSalesTracker() {
    ...
    function saveTransactionsForAnalytics() {
        ...
        try {
            const existing = localStorage.getItem('salestrack_cache');
            let existingTx = [];
            if (existing) {
                try {
                    existingTx = JSON.parse(existing);
            ...
            const merged = [...collectedTransactions];
            const existingIds = new Set(collectedTransactions.map(tx => tx.id));
            ...
            // Keep max 10000 transactions to prevent localStorage overflow
